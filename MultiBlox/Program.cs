    ...
    partial class Program
    {
        ...
        static string[]? RunHandleProc(string args)
        {
            ...
            using(Process pProcess = new Process())
            {
                ...
                {
                    Console.WriteLine($"ERROR: {ex.Message}"); // smth wtf happen lol
                }
        ...
        static string[] FindRobloxProcesses() =>
            ...
            .OrderByDescending(x => DateTime.Now - x.StartTime)
            .Select(robloxProc => robloxProc.Id.ToString())
            .ToArray();
        ...
        static bool FindAndCloseRobloxEvent(string pid, string handleName)
        {
            ...
            Console.WriteLine($"[PID:{pid}] Searching for {handleName}...");
            // call sysinternals to search for the roblox event handle 
            // see here for info on the args etc: 
            // https://learn.microsoft.com/en-us/sysinternals/downloads/handle#usage
            string[]? outlines = RunHandleProc($"-a -p {pid} {handleName} -v -accepteula"); // adding 'accepteula' arg bc it will fail if its never been accepted 
            if (outlines!=null) // if its null we got an error 
            {
                ...
                else if (outlines.Length>=2 && outlines[0].Trim()=="Process,PID,Type,Handle,Name") // if we find the event handle, we end up here
                {
                    ...
                    if (handlevalues.Length==5) // 5 heading names, so 5 values..
                    {
                        ...
                        Console.WriteLine($"[PID:{_pid}] {handleName} found! Closing...");
                        ...
                        outlines = RunHandleProc($"-c {hid} -y -p {pid}");
                        if (outlines!=null)
                        {
                            if (outlines.Length>=3 && outlines[2].Trim()=="Handle closed.") // confirming output so we know that it worked 
                            {
                                Console.WriteLine($"[PID:{_pid}] {handleName} closed!");
                                foundAndClosed = true;
                            }
                            else // otherwise smth must have gone wrong..
                            {
                                Console.WriteLine($"[PID:{_pid}] ERROR! Unexpected output! Output was: \r\n{string.Join('\n', outlines)}");
                            }
                        }
                    }
                    else
                    {
                        Console.WriteLine($"[PID:{pid}] ERROR! Unexpected number of handle values! Values were: \r\n{outlines[1].Trim()}");
                    }
                }
                else
                {
                    Console.WriteLine($"[PID:{pid}] ERROR! Unexpected output! Output was: \r\n{string.Join('\n', outlines)}");
                }
            }
            else
            {
        ...
        private static void OnEventArrived(object sender, EventArrivedEventArgs e)
        {
            ...
            {
                Console.WriteLine("");
                Console.WriteLine($"New Roblox Instance Detected! PID:{pid}");
        ...
        static void Main(string[] args)
        {
            ...
            if (pids.Length>0)
            {
                Console.WriteLine($"Found {pids.Length} Roblox instance(s) running. PIDS: {string.Join(", ", pids)}");
                ...
                {
                    Console.WriteLine($"[PID:{tpl.Item1}] {tpl.Item2} {(tpl.Item3 ? "CLOSED" : "NOT FOUND")}");
                    if (tpl.Item3)
                        foundAny = true;
                }
                Console.WriteLine("");
